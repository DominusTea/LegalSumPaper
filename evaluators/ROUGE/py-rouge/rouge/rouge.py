...
STOPWORDS_FILE="evaluators/assets/stopwords_el.txt"
...
class Rouge:
    ...
    REMOVE_CHAR_PATTERN = re.compile('[^A-Za-z0-9Α-Ωα-ωάέύήίόώϊΐϋΰ]')
    ...
    def __init__(self, metrics=None,
                 ...
                 apply_avg=True,
                 apply_best=False,
                 stemming=True, ignore_stopwords=True,
                 alpha=0.5, weight_factor=1.0,
                 ensure_compatibility=True, ensure_thesis_compatibility=False):
        """
        ...
        Args:
          ...
          ensure_thesis_compatibility: Use same sentence segmentor, tokenizer, stemmer as in the rest of our diploma thesis. If set to true, ignore 'ensure_compatibility'
        ...
        self.ensure_thesis_compatibility = ensure_thesis_compatibility
        ...
        if ensure_thesis_compatibility:
            self.nlp = adapt_spacy(spacy.load('el_core_news_sm', disable=['ner']))
        else:
            self.nlp=None
    ...
    @staticmethod
    def split_into_sentences(text, ensure_compatibility, ensure_thesis_compatibility=False, nlp=None,language='english',):
        """
        ...
        Returns:
          List of tokens of text (if ensure_thesis_compatibility == False)
          List of spacy.Spans otherwise
        """
        if ensure_thesis_compatibility:
            # spaCy sentence segmentation used in our thesis
            doc = nlp(text)
            return list(doc.sents)
        else:
            # regular NLTK english sentence segmentation
            if ensure_compatibility:
                return text.split('\n')
            else:
    ...
    @staticmethod
    def _split_into_words(sentences, nlp=None, ensure_thesis_compatibility=False):
        """
        Splits multiple sentences into words and flattens the result

        Args:
          sentences: list of string (Preprocessed!)

        Returns:
          A list of words (split by white space)
        """
        if ensure_thesis_compatibility:
            words_per_sentence_lst =  [[w.text for w in nlp(Rouge._remove_dupl_whitespace(sent))] for sent in sentences]
            # flatten into a single list
            res= [item for sublist in words_per_sentence_lst for item in sublist]
            return res
        else:
            # Modified from https://github.com/pltrdy/seq2seq/blob/master/seq2seq/metrics/rouge.py
    ...
    @staticmethod
    def _get_word_ngrams_and_length(n, sentences, nlp=None, ensure_thesis_compatibility=False):
        """
        ...
        tokens = Rouge._split_into_words(sentences, nlp=nlp, ensure_thesis_compatibility=ensure_thesis_compatibility)
        return Rouge._get_ngrams(n, tokens), tokens, len(tokens) - (n - 1)

    @staticmethod
    def _remove_dupl_whitespace(s):
      '''
    ...
    @staticmethod
    def _get_unigrams(sentences, nlp=None, ensure_thesis_compatibility=False):
        """
    ...
    @staticmethod
    def _compute_ngrams(evaluated_sentences, reference_sentences, n, nlp=None, ensure_thesis_compatibility=False):
        """
        ...
        evaluated_ngrams, _, evaluated_count = Rouge._get_word_ngrams_and_length(n, evaluated_sentences, nlp=nlp, ensure_thesis_compatibility=ensure_thesis_compatibility)
        reference_ngrams, _, reference_count = Rouge._get_word_ngrams_and_length(n, reference_sentences,nlp=nlp, ensure_thesis_compatibility=ensure_thesis_compatibility)
    ...
    @staticmethod
    def _compute_ngrams_lcs(evaluated_sentences, reference_sentences, weight_factor=1.0, nlp=None, ensure_thesis_compatibility=False):
        """
        ...
        def _mark_lcs(mask, dirs, m, n):
            ...
            return mask
        print("evaluated", evaluated_sentences)
        print("ref", reference_sentences)
        if len(evaluated_sentences) <= 0 or len(reference_sentences) <= 0:
            raise ValueError("Collections must contain at least 1 sentence.")

        evaluated_unigrams_dict, evaluated_count = Rouge._get_unigrams(evaluated_sentences, nlp=nlp, ensure_thesis_compatibility=ensure_thesis_compatibility)
        reference_unigrams_dict, reference_count = Rouge._get_unigrams(reference_sentences, nlp=nlp, ensure_thesis_compatibility= ensure_thesis_compatibility)
        ...
        for sample_id, (hypothesis_sentences, references_sentences) in enumerate(zip(all_hypothesis, all_references)):
            ...
            if self.apply_avg:
                ...
                for reference_sentences in references_sentences:
                    hypothesis_count, reference_count, overlapping_ngrams = Rouge._compute_ngrams_lcs(hypothesis_sentences, reference_sentences, self.weight_factor if use_w else 1.0, nlp=self.nlp, ensure_thesis_compatibility=self.ensure_thesis_compatibility)
                    total_hypothesis_ngrams_count += hypothesis_count
                    total_reference_ngrams_count += reference_count
                    total_ngrams_overlapping_count += overlapping_ngrams
    ...
    def _thesis_preproccess_sent(self, sentence, doStem=True):
        '''
        Preprocess each sentence according to our thesis preprocessing pipeline

        Args:
            sentence: spacy.Span
            doStem: True if stemming is enabled

        Returns:
            Return the preprocessed sentence (string)
        '''
        # function that recognises stopwords
        is_stopword_condition = lambda word: False if not self.ignore_stopwords else word.is_stop
        # print("with stemming: ", [word.lemma_.lower().strip() for word in sentence if not is_stopword_condition(word)])
        # print("without stemming: ", [word.text.lower().strip() for word in sentence if not is_stopword_condition(word)])
        if doStem:
            words = [word.lemma_.lower().strip() for word in sentence if not is_stopword_condition(word)]
        else:
            words = [word.text.lower().strip() for word in sentence if not is_stopword_condition(word)]
        #
        # print("!!!!!!!!!!!")
        # print([word.lemma_.lower().strip() for word in sentence if not is_stopword_condition(word)])
        # print('@@@@@@@@@@@@')
        # print([word.text.lower().strip() for word in sentence if not is_stopword_condition(word)])
    ...
    def _preprocess_summary_as_a_whole(self, summary):
        ...
        """
        sentences = Rouge.split_into_sentences(summary, self.ensure_compatibility, self.ensure_thesis_compatibility, nlp=self.nlp)


        if self.ensure_thesis_compatibility:
            # this means sentences is a list of spacy.Spans
            if self.limit_length:
                # By words
                if self.length_limit_type == 'words':
                    sentences_word_length = [len(sent) for sent in sentences]
                    total_word_length = sum([len(sent) for sent in sentences])

                    if total_word_length > self.length_limit:
                        # truncation is needed
                        # find partial sum of sentences' length
                        partial_sum_length = list(itertools.accumulate(sentences_word_length))
                        # find first index that exceeds word length limit
                        index = list(np.array(partial_sum_length) < self.length_limit+1).index(False)
                        if index != 0:
                            # keep more than one sentence
                            # find how many tokens to keep from last sentence
                            num_last_sentence_tokens = self.length_limit - partial_sum_length[index-1]
                            # and concatenate spaCy Spans accordingly
                            sentences = sentences[0:index] + [sentences[index][:num_last_sentence_tokens]]
                        else:
                            # keep just one sentence
                            num_last_sentence_tokens = self.length_limit
                            sentences=[sentences[0][:num_last_sentence_tokens]]
                else:
                    raise NotImplementedError("Only 'words' length limit type implemented")

            preprocessed_sents = [self._thesis_preproccess_sent(sent, self.stemming) for sent in sentences]
            # join into the same strng
            preprocessed_summary = " ".join(preprocessed_sents)
    ...
    def _preprocess_summary_per_sentence(self, summary):
        ...
        if self.ensure_thesis_compatibility:
            ...
            final_sentences = [self._thesis_preproccess_sent(sent, self.stemming) for sent in sentences]
            if self.ignore_stopwords:
                # remove empty sentences (which may happen if stopwords are ignored)
